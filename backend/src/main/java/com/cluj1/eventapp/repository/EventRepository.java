...
public interface EventRepository extends JpaRepository<Event, UUID> {
         ...
         *                     {@code null})
         * 
         * @param status       the event status (must not be {@code null})
         * @return matching events
         */
        @Query("SELECT e FROM Event e WHERE e.status = :status AND e.registrationEndDate >= :now AND (e.location = 'ALL' OR e.location = :userLocation)")
