...
public interface EventRepository extends JpaRepository<Event, UUID> {
        ...
        @Query("SELECT e FROM Event e WHERE e.status = :status AND (e.registrationEndDate IS NULL OR e.registrationEndDate >= :now) AND (e.location = 'ALL' OR e.location = :userLocation)")
        List<Event> findEligibleEvents(
                        @Param("now") OffsetDateTime now,
                        @Param("userLocation") EventLocation userLocation,
                        @Param("status") EventStatus status);

        @Query("SELECT e FROM Event e WHERE e.status = com.cluj1.eventapp.model.enums.EventStatus.PUBLISHED " +
                        "AND (e.registrationEndDate IS NULL OR e.registrationEndDate >= :now) " +
